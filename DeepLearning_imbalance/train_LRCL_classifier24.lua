...
local weight = torch.Tensor({24389/2189,24389/1686,24389/1513,24389/32,24389/292,24389/8677,24389/10000})
...
-- input(20,1,24,48),output(20,)

local model=nn.Sequential()

model:add(nn.SpatialConvolution(1,8,1,5))--(20,8,20,48)
model:add(nn.SpatialBatchNormalization(8))
model:add(nn.ReLU())
model:add(nn.SpatialDropout(0.3))
model:add(nn.SpatialBatchNormalization(8))
model:add(nn.SpatialConvolution(8,8,1,5))--(20,8,16,48)
model:add(nn.ReLU())
model:add(nn.SpatialDropout(0.3))
model:add(nn.Transpose({2,3}))--(20,16,8,48)
model:add(nn.View(20,16,-1))--(20,16,384)
model:add(nn.Sequencer(nn.FastLSTM(384,128)))
model:add(nn.Sequencer(nn.FastLSTM(128,64)))--20,16,64
...
batchLoader=require 'DataLoader_LRCL_fold'

local loader = batchLoader.create(batchSize,slidingWindowLength)

-- local checkpoint = torch.load('model_LRCL_classifier24_0708_train_final.t7')
...
function train(model)
  ...
  for i = 1, numEpochs do
    ...
    local seed = math.random(5)
    local batch = loader:nextBatch('train',seed)
    ...
    while batch ~= nil do
      ...
      batch = loader:nextBatch('train',seed)
      --batch=nil
    end
    ...
    if (checkpointEvery > 0 and i % checkpointEvery == 0) or i == numEpochs then
      ...
      local filename
      if i == numEpochs then
        filename = '%s_%s.t7' % {'model_LRCL_fold_classifier_'..slidingWindowLength..'_date_'..os.date("%m_%d"),'final'}
      else
        filename = '%s_%d.t7' % {'model_LRCL_fold_classifier_'..slidingWindowLength..'_date_'..os.date("%m_%d"), i}
      end
    ...
    if validLoss < LeastLoss then
      ...
      torch.save("best_model_LRCL_fold_win"..slidingWindowLength..".t7", checkpoint)
      print(string.format("New maxima : %f @ %f", LeastLoss, bestEpoch))
      waitTimes = 0
    else
      waitTimes = waitTimes + 1
      if waitTimes > 40 then break end
    end
  ...
  gnuplot.pngfigure('trainValLossAcc_LRCL_fold_win'..slidingWindowLength..'.png')
...
function test(model)
  ...
  filename='predict_LRCL_fold24_'..os.date("%m_%d")..'_'..os.time()..'.h5'
  ...
  myFile:write('scores',predScores:float())
	...
	os.execute('python calculate_LRCL.py '..filename..' LRCL_fold'..slidingWindowLength)
end
...
local ck=torch.load("best_model_LRCL_fold_win"..slidingWindowLength..".t7")
